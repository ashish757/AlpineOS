...
export const FinderApp = ({winInfo}: { winInfo: WindowState }) => {
  ...
  const emptySpaceContextMenu = attachContextMenu([
  ...
  useAppMenuBar(winInfo.id, {
    File: [
      { label: 'New Folder', action: handleNewFolder },
      { label: 'New File', action: handleNewFile },
      { divider: true, action: () => {} },
      { label: 'Close Window', action: () => dispatchAction(closeWindow(winInfo.id)) }
    ],
    Edit: [
      { label: 'Select All', action: () => setSelectedIds([...currentFolderContents.folders.map(f => f.id), ...currentFolderContents.files.map(f => f.id)]) },
      { label: 'Deselect All', action: () => setSelectedIds([]) }
    ],
    View: [
      { label: 'Back', action: navBack, disabled: isBackDisabled },
      { label: 'Forward', action: navFwd, disabled: isForwardDisabled }
    ],
    Help: [
      { label: 'Finder Help', action: () => alert("Welcome to Finder!") }
    ]
  }, [folderId, currentFolderContents.folders, currentFolderContents.files, isBackDisabled, isForwardDisabled, selectedIds]);
    ...
    <>
      <div className="flex h-full w-full flex-col bg-[#1c1c1e] text-slate-200 font-sans" onClick={() => setSelectedIds([])}>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex flex-1 flex-col bg-[#1e1e1e] pt-1 pl-1" style={{ justifyContent: 'space-between' }} {...emptySpaceContextMenu}>
            <div className="grid grid-cols-[repeat(auto-fill,minmax(80px,1fr))] gap-2">
              {currentFolderContents.folders.map((fItem) => (
                <div
                  ...
                  {...getItemContextMenu(fItem.id, fItem.name, 'folder')}
              ...
              {currentFolderContents.files.map((fItem) => (
                <div
                  ...
                  {...getItemContextMenu(fItem.id, fItem.name, 'file')}
                  className={`group flex cursor-pointer flex-col items-center gap-2 rounded-lg border border-transparent transition-all ${selectedIds.includes(fItem.id) ? "bg-white/10 border-white/20" : "hover:bg-white/5"
                    }`}
                >
